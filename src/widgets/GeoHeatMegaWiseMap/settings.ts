...
import {polygonFilterGetter, KEY} from '../Utils/Map';
import {dimensionGetter, measureGetter} from '../../utils/WidgetHelpers';
import {vegaHeatMapGen} from '../Utils/Vega';
...
const geoHeatMapConfigHandler: ConfigHandler<GeoHeatMapConfig> = config => {
  ...
  const dm = [lon, lat, color];
  const as = dm.map((m: any) => m.as);
  const rect = as.map((a: string) => `rect.${a}`);

  // gen vega
  newConfig.renderSelect = `plot_scatter_2d(${rect.join(' , ')}, '${vegaHeatMapGen(newConfig)}')`;
